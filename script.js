...
// --- SCROLL STORYTELLING: cinematic scenes, parallax depth + film layer ---
(function () {
    const reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

    // Film grain + vignette over the whole page.
    const film = document.createElement('div');
    film.className = 'film-layer';
    document.body.appendChild(film);

    // Scene master — a section plays its entrance once when it arrives.
    const sections = document.querySelectorAll('.cinema-section');
    if (sections.length) {
        ...
        sections.forEach(s => sceneObs.observe(s));
    }

    // Parallax depth on background layers (skipped for reduced motion).
    const layers = document.querySelectorAll('.cinema-bg-layer');
    ...
    const update = () => {
        ...
        layers.forEach(layer => {
            const sec = layer.closest('.cinema-section') || layer.parentElement;
            if (!sec) return;
            const r = sec.getBoundingClientRect();
            if (r.bottom < 0 || r.top > vh) return;
            const mid = (r.top + r.height / 2) - vh / 2;
            const y = Math.max(-40, Math.min(40, mid * 0.12));
            layer.style.transform = 'translate3d(0, ' + y.toFixed(1) + 'px, 0) scale(1.06)';
        });
    };
    const onScroll = () => {
